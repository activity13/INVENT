...
app.on('ready', () => {
    ...
    win.loadURL(`file://${__dirname}/routes/almacen.html`);
...
ipcMain.on('editar-stock-total', async (e, arg) => {
    const objeto = await ProductosExremos.find({_id: arg.idAEditar})
    console.log(objeto.Codf)
...
ipcMain.on('editar-almacen', async (e, arg) => {
    const sotckAlmacen = await ProductosExremos.findByIdAndUpdate(arg.idAEditar, {
        ...
        Market: Number(arg.valorStock) - Number(arg.inputQty)
    }, {
        new: true
    })
    console.log(arg.valorMarket)
...
ipcMain.on('salida', async (e, arg) =>  {
    const salidaAlmacen = await ProductosExremos.findByIdAndUpdate(arg.idAEditar, {
        ...
        Market: Number(arg.valorMarket) + Number(arg.inputQty)
        }, {
            new: true
        })
     ...
     const nuevoRegistro = await ProductosExremos.findOne({
        _id: arg.idAEditar
    })
    const salidaRegistro = new movimintosExtremos({
        Codf: nuevoRegistro.Codf,
        Descr: nuevoRegistro.Descr,
        Tipo: 'Salida a Market',
        Stock: nuevoRegistro.Stock,
        Stock_Final: nuevoRegistro.Stock,
        Almacen: arg.valorPicos - arg.inputQty,
        Market: (nuevoRegistro.Market),
        Cantidad: '-' + arg.inputQty,
        fecha: new Date
    })
    console.log(salidaRegistro)
    await salidaRegistro.save()
